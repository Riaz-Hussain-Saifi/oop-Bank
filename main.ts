...
console.log(
  chalk.yellow(
    `
 \n\t Welcome to the ${chalk.red("Saifi Developer's")} ${chalk.green(
      "OOP"
    )} ${chalk.blue("Bank")}!\n
 `
  )
...
let PinCode: number = 2024; // Initial PIN code
let myBalance: number = 10000; // Assuming an initial balance
const accountInfo = {
  name: "Riaz Hussain",
...
async function moneyTransfer() {
  ...
  console.log(
    chalk.greenBright(
      `\n\tYou have transferred ${chalk.cyanBright(
        transfer.amount
      )} to account ${chalk.yellowBright(transfer.accountNumber)}\n`
    )
  );
  console.log(
    chalk.greenBright(
...
function miniStatementDisplay() {
  ...
  miniStatement.forEach((transaction) => {
    console.log(
      chalk.greenBright(
        `\t${transaction.date}\t${chalk.cyanBright(
          transaction.description
        )}\t${chalk.whiteBright(transaction.amount)}`
      )
    );
  });
  console.log(chalk.greenBright("\n"));
...
function accountInformation() {
  ...
  console.log(
    chalk.greenBright(`\tName: ${chalk.whiteBright(accountInfo.name)}`)
  );
  console.log(
    chalk.greenBright(
      `\tAccount Number: ${chalk.whiteBright(accountInfo.accountNumber)}`
    )
  );
  console.log(
    chalk.greenBright(`\tBranch: ${chalk.whiteBright(accountInfo.branch)}`)
  );
  console.log(
    chalk.greenBright(`\tIFSC: ${chalk.whiteBright(accountInfo.ifsc)}`)
  );
  console.log(
    chalk.greenBright(
      `\tCurrent Date and Time: ${chalk.whiteBright(
